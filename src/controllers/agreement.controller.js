...
const { generateUrlSafeToken, hashToken, hashContent } = require('../utils/encryption.utils');
...
exports.createAgreement = catchAsync(async (req, res, next) => {
  ...
  if (!debtor) {
    return next(
      new AppError('Debtor not found. Ask them to register on TrustLedger first.', 404)
    );
  }
  if (String(debtor._id) === String(req.user._id)) {
    return next(new AppError('You cannot create an agreement with yourself.', 400));
  }
  ...
  const agreement = await Agreement.create({
    ...
    statusHistory: [
      { status: 'pending_debtor_verification', changedBy: creditor._id, reason: 'Agreement created' },
    ],
  });
  ...
  sendCreated(res, { message: 'Agreement created successfully.', data: agreement });
});
...
exports.getMyAgreements = catchAsync(async (req, res) => {
  ...
  else filter.$or = [{ 'creditor.user': req.user._id }, { 'debtor.user': req.user._id }];
...
exports.getAgreement = catchAsync(async (req, res, next) => {
  const agreement = await Agreement.findById(req.params.id)
    .populate('creditor.user', 'firstName lastName avatar trustScore phone email')
    .populate('debtor.user', 'firstName lastName avatar trustScore phone email')
    .populate('disputes');
  ...
  if (!isParty) return next(new AppError('Not authorised to view this agreement.', 403));

  sendSuccess(res, { data: agreement });
});

// ── Initiate In-Person Verification (generate OTP) ───────────────────────────
exports.initiateInPersonVerification = catchAsync(async (req, res, next) => {
  const agreement = await Agreement.findById(req.params.id);
  if (!agreement) return next(new AppError('Agreement not found.', 404));

  if (String(agreement.creditor.user) !== String(req.user._id)) {
    return next(new AppError('Only the creditor can initiate in-person verification.', 403));
  }
  if (agreement.verificationMode !== 'in_person') {
    return next(new AppError('This agreement uses remote verification.', 400));
  }

  const otp = await smsService.generateAgreementOTP(agreement._id, req.user.phone);

  agreement.inPersonVerification.otpExpiry = new Date(
    Date.now() + parseInt(process.env.OTP_EXPIRE_MINUTES || 10) * 60 * 1000
  );
  await agreement.save();

  sendSuccess(res, {
    message: `OTP sent to your phone. Share it with the debtor.`,
    data: {
      otpExpiry: agreement.inPersonVerification.otpExpiry,
...
exports.completeInPersonVerification = catchAsync(async (req, res, next) => {
  ...
  if (String(agreement.debtor.user) !== String(req.user._id)) {
    return next(new AppError('Only the debtor can complete in-person verification.', 403));
  }

  // Verify OTP
  const otpResult = await smsService.verifyAgreementOTP(agreement._id, otp);
  if (!otpResult.valid) return next(new AppError(otpResult.reason, 400));
...
exports.creditorSign = catchAsync(async (req, res, next) => {
  ...
  if (agreement.debtor.signedAt) {
    ...
    await User.findByIdAndUpdate(agreement.debtor.user, {
      $inc: { 'stats.totalAgreementsAsDebtor': 1, 'stats.totalAmountBorrowed': agreement.principalAmount },
    });
  ...
  await agreement.save();
  sendSuccess(res, { message: 'Agreement signed.', data: { status: agreement.status } });
});
...
exports.generateRemoteVerificationLink = catchAsync(async (req, res, next) => {
  ...
  if (String(agreement.creditor.user) !== String(req.user._id)) {
    return next(new AppError('Only the creditor can generate the verification link.', 403));
  }
  if (agreement.verificationMode !== 'remote') {
    return next(new AppError('This agreement uses in-person verification.', 400));
  }
  ...
  agreement.remoteVerification.linkExpiry = new Date(
    Date.now() + parseInt(process.env.OTP_LINK_EXPIRE_HOURS || 24) * 60 * 60 * 1000
  );
  await agreement.save();
...
exports.completeRemoteVerification = catchAsync(async (req, res, next) => {
  ...
  const [idPhotoUrl, selfieUrl] = await Promise.all([
    cloudinaryService.uploadIdFront(req.files.idFront[0].buffer, `remote_${agreement._id}`),
    cloudinaryService.uploadSelfie(req.files.selfie[0].buffer, `remote_${agreement._id}`),
  ]);

  // Face matching
  const matchResult = await faceMatchService.compareFaces(idPhotoUrl, selfieUrl);
...
exports.markAsDefaulted = catchAsync(async (req, res, next) => {
  ...
  if (String(agreement.creditor.user) !== String(req.user._id)) {
    return next(new AppError('Only the creditor can mark an agreement as defaulted.', 403));
  }
  if (agreement.status !== 'active') {
    return next(new AppError('Only active agreements can be marked as defaulted.', 400));
  }
  ...
  notificationService.emitTrustScoreUpdate(io, agreement.debtor.user, scoreResult);
  ...
  sendSuccess(res, { message: 'Agreement marked as defaulted.', data: agreement });
});
...
exports.cancelAgreement = catchAsync(async (req, res, next) => {
  ...
  if (!isCreditor && req.user.role !== 'admin') {
    return next(new AppError('Only the creditor or admin can cancel an agreement.', 403));
  }
  if (!['draft', 'pending_debtor_verification'].includes(agreement.status)) {
    return next(new AppError('Only draft or pending agreements can be cancelled.', 400));
  }
