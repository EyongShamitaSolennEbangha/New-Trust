...
exports.submitIdentityVerification = catchAsync(async (req, res, next) => {
  ...
  const tempIdPath = path.join(
    __dirname,
    "../temp",
    `id_front_${Date.now()}.jpg`,
  );
  const tempSelfiePath = path.join(
    __dirname,
    "../temp",
    `selfie_${Date.now()}.jpg`,
  );

  fs.writeFileSync(tempIdPath, idFrontFile.buffer);
  fs.writeFileSync(tempSelfiePath, selfieFile.buffer);
  ...
  try {
    ...
    // Clean up temp files
    fs.unlinkSync(tempIdPath);
    fs.unlinkSync(tempSelfiePath);

    if (!matchResult.matched) {
      return next(
        new AppError(
          `Face verification failed (${matchResult.confidence}% match — 80% required).`,
          400,
        ),
      );
    }
    ...
    const user = await User.findByIdAndUpdate(req.user._id, {
      ...
      "idDocument.type": idType,
      "idDocument.number": encrypt(idNumber),
      "idDocument.frontPhoto": idFrontFile.buffer.toString("base64"), // or upload to Cloudinary
      "idDocument.selfiePhoto": selfieFile.buffer.toString("base64"),
      "idDocument.verifiedAt": new Date(),
      "idDocument.aiMatchScore": matchResult.score * 100,
    });
    ...
    // Clean up on error
    if (fs.existsSync(tempIdPath)) fs.unlinkSync(tempIdPath);
...
exports.updateProfile = catchAsync(async (req, res, next) => {
  ...
  if (hasForibdden) {
    return next(
      new AppError(
        "You cannot update restricted fields through this route.",
        400,
      ),
    );
  }
...
exports.submitIdentityVerification = catchAsync(async (req, res, next) => {
  ...
  let idBackUrl = null;
  if (req.files?.idBack) {
    idBackUrl = await cloudinaryService.uploadIdBack(
  ...
  if (!faceDetection.hasExactlyOneFace) {
    return next(
      new AppError(
        "Selfie must contain exactly one clearly visible face.",
        400,
      ),
    );
  }
...
exports.getUserStats = catchAsync(async (req, res) => {
  const user = await User.findById(req.user._id).select(
    "stats trustScore behaviorMetrics",
  );
  sendSuccess(res, { data: user });
});
