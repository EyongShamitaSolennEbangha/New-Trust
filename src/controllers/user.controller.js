...
exports.updateProfile = catchAsync(async (req, res, next) => {
  ...
  const hasForbidden = forbidden.some((f) => req.body[f]);
  if (hasForbidden) {
    return next(
      new AppError("You cannot update restricted fields through this route.", 400),
    );
  }
...
// ── Identity Verification (Cloudinary version – correct one) ─────────────────
...
exports.approveKyc = catchAsync(async (req, res, next) => {
  ...
  const io = req.app.get('io');
  const scoreResult = await trustScoreService.updateUserTrustScore(user._id, 'KYC approved by admin');
  notificationService.emitTrustScoreUpdate(io, user._id, scoreResult);
  
  sendSuccess(res, { message: 'KYC approved successfully', data: user });
});
...
exports.rejectKyc = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findByIdAndUpdate(
    req.params.id,
    {
      ...
      notes: `KYC rejected: ${reason || 'No reason provided'}`,
    },
    { new: true }
  );
  if (!user) return next(new AppError('User not found.', 404));
  sendSuccess(res, { message: 'KYC rejected', data: user });
});
