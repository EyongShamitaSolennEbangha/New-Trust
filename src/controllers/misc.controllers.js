...
exports.getMyTrustScore = catchAsync(async (req, res) => {
  const user = await User.findById(req.user._id).select(
    "trustScore isIdentityVerified stats",
  );
...
exports.getUserTrustScore = catchAsync(async (req, res, next) => {
  const user = await User.findById(req.params.id).select(
    "firstName lastName trustScore isIdentityVerified",
  );
...
exports.getTrustScoreHistory = catchAsync(async (req, res) => {
  const user = await User.findById(req.user._id).select(
    "trustScore.history trustScore.score trustScore.level",
  );
...
exports.createDispute = catchAsync(async (req, res, next) => {
  ...
  if (req.files?.length) {
    for (let i = 0; i < req.files.length; i++) {
      const url = await cloudinaryService.uploadDisputeEvidence(
        req.files[i].buffer,
        `new_${Date.now()}`,
        i,
      );
...
exports.getMyDisputes = catchAsync(async (req, res) => {
  ...
  const skip = (page - 1) * limit;
  const filter = {
    $or: [{ initiatedBy: req.user._id }, { respondent: req.user._id }],
  };
...
exports.addDisputeMessage = catchAsync(async (req, res, next) => {
  ...
  if (!["open", "under_review"].includes(dispute.status)) {
    return next(
      new AppError("Cannot add messages to a resolved dispute.", 400),
    );
  }
...
exports.resolveDispute = catchAsync(async (req, res, next) => {
  ...
  if (!validOutcomes.includes(outcome))
    return next(new AppError("Invalid outcome.", 400));
...
exports.verifyAgreementPublic = catchAsync(async (req, res, next) => {
  ...
  const agreement = await Agreement.findOne({ agreementId }).select(
    "agreementId status documentHash principalAmount totalAmountDue remainingBalance currency dueDate createdAt activatedAt lastPaymentAt disputes",
  );
  if (!agreement) return next(new AppError("Agreement not found.", 404));
  ...
  sendSuccess(res, {
    data: {
      ...
      totalAmountDue: agreement.totalAmountDue,
      remainingBalance: agreement.remainingBalance,
      currency: agreement.currency,
      dueDate: agreement.dueDate,
      lastPaymentAt: agreement.lastPaymentAt,
      isOverdue:
        agreement.status === "active" && agreement.dueDate < new Date(),
      paymentProgress: agreement.paymentProgress,
      disputeCount: agreement.disputes?.length || 0,
...
exports.getUserPublicProfile = catchAsync(async (req, res, next) => {
  const user = await User.findById(req.params.id).select(
    "firstName lastName trustScore.score trustScore.level isIdentityVerified stats.completedAgreements stats.defaultedAgreements isDefaulter",
  );
  if (!user) return next(new AppError("User not found.", 404));
  sendSuccess(res, { data: user });
});
...
exports.removeDefaulter = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError("User not found.", 404));
  sendSuccess(res, {
    message: "User removed from defaulter list.",
    data: user,
  });
});
